...
const preventMutualExclusion = () => {
  ...
  if (hasTCF && hasTEG) {
    ...
    const lastSelected = selectedMeasTypes.value[selectedMeasTypes.value.length - 1];
    if (lastSelected === 'TEG') {
      selectedMeasTypes.value = selectedMeasTypes.value.filter(type => type !== 'TCF');
    } else if (lastSelected === 'TCF') {
      selectedMeasTypes.value = selectedMeasTypes.value.filter(type => type !== 'TEG');
    }
  }
}

// selectedMeasTypes의 변화를 감지하여 중복 선택을 방지하고, emitUpdate 함수를 호출
watch(selectedMeasTypes, (newVal) => {
